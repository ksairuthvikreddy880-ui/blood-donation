    ...
    <nav className="fixed top-0 left-0 right-0 z-40 bg-background/80 backdrop-blur-xl border-b border-border/50">
      <div className="container max-w-6xl mx-auto flex items-center justify-between h-16 px-4">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <Link to="/auth" className="px-5 py-2.5 text-sm font-semibold bg-primary text-primary-foreground rounded-xl hover:opacity-90 transition-opacity">
            Register Now
          </Link>
        </div>
        ...
        {open && (
          ...
          >
            <div className="px-4 py-6 space-y-4">
              ...
              <Link to="/auth" className="block w-full text-left text-foreground font-medium" onClick={() => setOpen(false)}>Sign In</Link>
              <Link to="/auth" className="block w-full text-center py-3 bg-primary text-primary-foreground rounded-xl font-semibold" onClick={() => setOpen(false)}>
                Register Now
              </Link>
            </div>
          </motion.div>
